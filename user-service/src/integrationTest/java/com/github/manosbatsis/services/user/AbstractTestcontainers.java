...
public abstract class AbstractTestcontainers {
    ...
    private static final GenericContainer<?> discoveryServiceContainer = new GenericContainer<>("manosbatsis/discovery-service:1.0.0");
    private static final GenericContainer<?> configServiceContainer = new GenericContainer<>("manosbatsis/config-service:1.0.0");
    private static final GenericContainer<?> eventServiceContainer = new GenericContainer<>("manosbatsis/event-service:1.0.0");
    private static final GenericContainer<?> emailServiceContainer = new GenericContainer<>("manosbatsis/email-service:1.0.0");
    ...
    private static void dynamicProperties(DynamicPropertyRegistry registry) {
        ...
        greenMailContainer.withNetwork(network)
            .withNetworkAliases("greenmail")
            .waitingFor(Wait.forLogMessage(".*Starting GreenMail standalone.*", 1))
            .withEnv("GREENMAIL_OPTS", "-Dgreenmail.setup.test.smtp -Dgreenmail.hostname=0.0.0.0 -Dgreenmail.users=user:password")
            .withExposedPorts(3025)
            .start();

        // config-service
        configServiceContainer.withNetwork(network)
            .withNetworkAliases("config-service")
            .withExposedPorts(CONFIG_SERVICE_EXPOSED_PORT)
            .waitingFor(Wait.forHttp("/user/default")
                .forPort(CONFIG_SERVICE_EXPOSED_PORT).forStatusCode(200).withStartupTimeout(STARTUP_TIMEOUT))
            .start();

        Slf4jLogConsumer logConsumer = new Slf4jLogConsumer(log);
        // discovery-service
        discoveryServiceContainer.withNetwork(network)
            ...
            .waitingFor(Wait.forHttp("/eureka/v2/apps")
                .forPort(DISCOVERY_SERVICE_EXPOSED_PORT).forStatusCode(200).withStartupTimeout(STARTUP_TIMEOUT))
            .start();

        // email-service
        emailServiceContainer.withNetwork(network)
            ...
            .dependsOn(configServiceContainer, schemaRegistryContainer, kafkaContainer, greenMailContainer)
            .waitingFor(Wait.forHttp("/actuator/health")
                .forPort(EMAIL_SERVICE_EXPOSED_PORT).forStatusCode(200).withStartupTimeout(STARTUP_TIMEOUT))
//            .withLogConsumer(new Consumer<OutputFrame>() {
//                @Override
//                public void accept(OutputFrame outputFrame) {
//                    System.err.println("EMAIL-SERVICE: " + outputFrame.getUtf8StringWithoutLineEnding());
//                }
//            })
            .start();
...
//                @Override
//                public void accept(OutputFrame outputFrame) {
//                    System.err.println("EVENT-SERVICE: " + outputFrame.getUtf8StringWithoutLineEnding());
//                }
//            })
            .start();
